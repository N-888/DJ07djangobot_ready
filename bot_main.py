...
if __name__ == '__main__':
    ...
    # Импортируем модуль time, чтобы можно было делать паузу перед повторной попыткой запуска бота.
    import time

    # Проверяем, что этот файл запущен напрямую, а не импортирован из другого Python-файла.
    if __name__ == '__main__':
        # Запускаем бесконечный цикл, чтобы бот автоматически перезапускался после временных сбоев сети.
        while True:
            # Пробуем выполнить запуск бота внутри блока try, чтобы программа не завершалась при ошибке.
            try:
                # Печатаем в терминал сообщение о том, что бот сейчас запускается и готов ждать новые сообщения.
                print('Telegram-бот запущен и ожидает сообщения...')

                # Печатаем в терминал адрес Django API, чтобы было удобно видеть, куда бот отправляет запросы.
                print(f'REGISTER_URL = {REGISTER_URL}')

                # Запускаем бесконечный опрос Telegram-серверов для получения новых сообщений от пользователей.
                # skip_pending=False оставляет возможность получить ожидающие обновления, если они уже есть.
                # timeout=20 задает время ожидания ответа от сети в секундах.
                # long_polling_timeout=20 задает время долгого ожидания новых обновлений от Telegram.
                bot.infinity_polling(skip_pending=False, timeout=20, long_polling_timeout=20)

            # Перехватываем любую ошибку, чтобы бот не завершился полностью, а попытался стартовать снова.
            except Exception as error:
                # Печатаем в терминал заголовок ошибки, чтобы было понятно, что проблема связана именно с Telegram или сетью.
                print('Ошибка связи с Telegram:')

                # Печатаем сам текст ошибки, чтобы можно было увидеть точную причину сбоя.
                print(error)

                # Сообщаем в терминал, что программа не закрывается, а через несколько секунд попробует запуститься заново.
                print('Повторная попытка через 5 секунд...')

                # Делаем паузу на 5 секунд перед новой попыткой подключения, чтобы не создавать слишком частые запросы.
                time.sleep(5)